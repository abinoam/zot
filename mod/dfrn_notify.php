...
function dfrn_notify_post(&$a) {
	...
	if(isset($sugg[0]['child'][NAMESPACE_DFRN])) {
		...
		$r = q("INSERT INTO `intro` ( `uid`, `fid`, `contact_id`, `note`, `hash`, `datetime`, `blocked` )
	...
	if(isset($rawmail[0]['child'][NAMESPACE_DFRN])) {
		...
		$msg['from_name'] = notags(unxmlify($base['sender'][0]['child'][NAMESPACE_DFRN]['name'][0]['data']));
		$msg['from_photo'] = notags(unxmlify($base['sender'][0]['child'][NAMESPACE_DFRN]['avatar'][0]['data']));
		$msg['from_url'] = notags(unxmlify($base['sender'][0]['child'][NAMESPACE_DFRN]['uri'][0]['data']));
		$msg['contact_id'] = $importer['id'];
		...
		if($importer['notify_flags'] & NOTIFY_MAIL) {
	...
	foreach($feed->get_items() as $item) {
		...
		if($is_reply) {
			if($feed->get_item_quantity() == 1) {
				...
				$datarray['contact_id'] = $importer['id'];
				...
				if($posted_id) {
					...
					if(! $is_like) {
						$r1 = q("UPDATE `item` SET `last_child` = 0, `changed` = '%s' WHERE `uid` = %d AND `parent` = %d",
							dbesc(datetime_convert()),
							intval($importer['importer_uid']),
							intval($r[0]['parent'])
						);

						$r2 = q("UPDATE `item` SET `last_child` = 1, `changed` = '%s' WHERE `uid` = %d AND `id` = %d LIMIT 1",
							dbesc(datetime_convert()),
							intval($importer['importer_uid']),
							intval($posted_id)
						);
					...
					if($posted_id && $parent) {
						...
						if((! $is_like) && ($importer['notify_flags'] & NOTIFY_COMMENT) && (! $importer['self'])) {
							push_lang($importer['language']);
							require_once('bbcode.php');
							$from = stripslashes($datarray['author_name']);
							...
							$email_html_body_tpl = replace_macros($tpl,array(
								...
								'$sitename'		=> $a->config['sitename'],			// name of this site
								'$siteurl'		=> z_path(),				// descriptive url of this site
								'$thumb'		=> $datarray['author_avatar'],			// thumbnail url for sender icon
								'$email'		=> $importer['email'],				// email address to send to
								'$url'			=> $datarray['author_link'],			// full url for the site
								'$from'			=> $from,					// name of the person sending the message
								'$body'			=> $msg['htmlversion'],			// html version of the message
								'$display'		=> z_path() . '/display/' . $importer['nickname'] . '/' . $posted_id,
							));
				...
				if(($datarray['type'] != 'activity') && ($importer['notify_flags'] & NOTIFY_COMMENT)) {

					$myconv = q("SELECT `author_link`, `author_avatar` FROM `item` WHERE `parent_uri` = '%s' AND `uid` = %d AND `parent` != 0 ",
						dbesc($parent_uri),
						intval($importer['importer_uid'])
					);
					if(count($myconv)) {
						$importer_url = z_path() . '/profile/' . $importer['nickname'];
						foreach($myconv as $conv) {
							if(! link_compare($conv['author_link'],$importer_url))
								continue;
