...
function events_content(&$a) {
	...
	$o .= replace_macros($tpl,array(
		...
		'$phototab' => z_path() . '/photos/' . $a->user['nickname'],
		'$status' => t('Status'),
		'$profile' => t('Profile'),
		'$photos' => t('Photos'),
		'$events' => t('Events') ,
	...
	if($mode == 'view') {
		...
		$r = q("SELECT `event`.*, `item`.`id` AS `itemid`,`item`.`plink`,
			`item`.`author_name`, `item`.`author_avatar`, `item`.`author_link` FROM `event` LEFT JOIN `item` ON `item`.`event_id` = `event`.`id` 
			WHERE `event`.`uid` = %d
		...
		if(count($r)) {
			...
			foreach($r as $rr) {
				...
				if(! x($links,$j)) 
					$links[$j] = z_path() . '/' . $a->cmd . '#link-' . $j;
			}
		}


		$o .= '<div id="new-event-link"><a href="' . z_path() . '/events/new' . '" >' . t('Create New Event') . '</a></div>';
		$o .= '<div id="event-calendar-wrapper">';

		$o .= '<a href="' . z_path() . '/events/' . $prevyear . '/' . $prevmonth . '" class="prevcal"><div id="event-calendar-prev" class="icon prev" title="' . t('Previous') . '"></div></a>';
		$o .= cal($y,$m,$links, ' eventcal');

		$o .= '<a href="' . z_path() . '/events/' . $nextyear . '/' . $nextmonth . '" class="nextcal"><div id="event-calendar-next" class="icon next" title="' . t('Next') . '"></div></a>';
		$o .= '</div>';
		...
		if(count($r)) {
			...
			foreach($r as $rr) {
				...
				$o .= ((! $rr['cid']) ? '<a href="' . z_path() . '/events/event/' . $rr['id'] . '" title="' . t('Edit event') . '" class="edit-event-link icon pencil"></a>' : '');
				if($rr['plink'])
	...
	if($mode === 'edit' || $mode === 'new') {
		...
		$a->page['htmlhead'] .= replace_macros($htpl,array('$baseurl' => z_path()));
