...
function network_init(&$a) {
	...
	if(($a->argc > 1 && $a->argv[1] === 'new') || ($a->argc > 2 && $a->argv[2] === 'new') || x($_GET,'search'))
		$a->page['aside'] .= '<a href="' . z_path() . '/' . str_replace('/new', '', $a->cmd) . ((x($_GET,'cid')) ? '?cid=' . $_GET['cid'] : '') . '">' . t('View Conversations') . '</a></div>';
	else { 
		$a->page['aside'] .= '<a href="' . z_path() . '/' . $a->cmd . '/new' . ((x($_GET,'cid')) ? '/?cid=' . $_GET['cid'] : '') . '">' . t('View New Items') . '</a></div>';

		if(x($_GET,'star'))
			$a->page['aside'] .= '<div id="network-star-link">'
				. '<a class="network-star" href="' . z_path() . '/' . $a->cmd 
...
function network_content(&$a, $update = 0) {
	...
	if($group) {
		...
		$sql_extra = " AND `item`.`parent` IN ( SELECT `parent` FROM `item` WHERE `id` = `parent` $star_sql AND ( `contact_id` IN ( $contact_str ) OR `allow_gid` REGEXP '<" . intval($group) . ">' )) ";
	...
	elseif($cid) {
		...
		if(count($r)) {
			$sql_extra = " AND `item`.`parent` IN ( SELECT `parent` FROM `item` WHERE `id` = `parent` $star_sql AND `contact_id` IN ( " . intval($cid) . " )) ";
