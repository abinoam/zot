...
function message_post(&$a) {
	...
	$hash = random_string();
 	$uri = 'urn:X-dfrn:' . z_path() . ':' . local_user() . ':' . $hash ;

	if(! strlen($replyto))
		$replyto = $uri;

	$r = q("INSERT INTO `mail` ( `uid`, `from_name`, `from_photo`, `from_url`, 
		`contact_id`, `title`, `body`, `seen`, `replied`, `uri`, `parent_uri`, `created`)
...
function message_content(&$a) {
	...
	$myprofile = z_path() . '/profile/' . $a->user['nickname'];
	...
	if(($a->argc == 3) && ($a->argv[1] === 'drop' || $a->argv[1] === 'dropconv')) {
		...
		if($cmd === 'drop') {
			...
			goaway(z_path() . '/message' );
		}
		else {
			$r = q("SELECT `parent_uri` FROM `mail` WHERE `id` = %d AND `uid` = %d LIMIT 1",
				intval($a->argv[2]),
				intval(local_user())
			);
			if(count($r)) {
	...
	if(($a->argc == 1) || ($a->argc == 2 && $a->argv[1] === 'sent')) {
		...
		$r = q("SELECT count(*) AS `total` FROM `mail` 
			WHERE `mail`.`uid` = %d AND `from_url` $eq '%s' GROUP BY `parent_uri` ORDER BY `created` DESC",
			intval(local_user()),
			dbesc($myprofile)
		);
		if(count($r))
		...
		$r = q("SELECT max(`mail`.`created`) AS `mailcreated`, min(`mail`.`seen`) AS `mailseen`, 
			...
			FROM `mail` LEFT JOIN `contact` ON `mail`.`contact_id` = `contact`.`id` 
			WHERE `mail`.`uid` = %d AND `from_url` $eq '%s' GROUP BY `parent_uri` ORDER BY `created` DESC  LIMIT %d , %d ",
			intval(local_user()),
			dbesc($myprofile),
		...
		foreach($r as $rr) {
			$o .= replace_macros($tpl, array(
				'$id' => $rr['id'],
				'$from_name' =>$rr['from_name'],
				'$from_url' => z_path() . '/redir/' . $rr['contact_id'],
				'$sparkle' => ' sparkle',
				'$from_photo' => $rr['thumb'],
	...
	if(($a->argc > 1) && (intval($a->argv[1]))) {
		...
		if(count($r)) { 
			$contact_id = $r[0]['contact_id'];
			$messages = q("SELECT `mail`.*, `contact`.`name`, `contact`.`url`, `contact`.`thumb` 
				FROM `mail` LEFT JOIN `contact` ON `mail`.`contact_id` = `contact`.`id` 
				WHERE `mail`.`uid` = %d AND `mail`.`parent_uri` = '%s' ORDER BY `mail`.`created` ASC",
				intval(local_user()),
		...
		foreach($messages as $message) {
			if($message['from_url'] == $myprofile) {
				$from_url = $myprofile;
				$sparkle = '';
			}
			else {
				$from_url = z_path() . '/redir/' . $message['contact_id'];
				$sparkle = ' sparkle';
			}
			$o .= replace_macros($tpl, array(
				'$id' => $message['id'],
