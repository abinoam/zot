...
function _abs_url($s){
	$a = get_app();
	return preg_replace("|href=(['\"])([^h][^t][^t][^p])|", "href=\$1".z_path()."/\$2", $s);
}
...
function widgets_content(&$a) {
	...
	if ($a->argv[2]=="cb"){
		...
		if (isset($_GET['p']) && local_user()==$conf['uid'] ) {
			...
			$o .= '<a href="'.z_path().'/settings/addon">'. t("Plugin Settings") .'</a>';
		...
		$o .= replace_macros($script, array(
			'$entrypoint' => z_path()."/widgets/".$a->argv[1]."/cb/",
			'$key' => $conf['key'],
			'$widget_id' => 'f9k_'.$a->argv[1]."_".time(),
			'$loader' => z_path()."/images/rotator.gif",
		...
		if (isset($_GET['p'])) {
			...
			.htmlspecialchars('<script src="'.z_path().'/widgets/'.$a->argv[1].'?k='.$conf['key'])
			.$jsargs
