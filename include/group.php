...
function group_rmv_member($uid,$name,$member) {
	...
	if(! ( $uid && $gid && $member))
		return false;
	$r = q("DELETE FROM `group_member` WHERE `uid` = %d AND `gid` = %d AND `contact_id` = %d LIMIT 1 ",
		intval($uid),
		intval($gid),
		intval($member)
	);
...
function group_add_member($uid,$name,$member) {
	...
	$r = q("SELECT * FROM `group_member` WHERE `uid` = %d AND `id` = %d AND `contact_id` = %d LIMIT 1",	
		intval($uid),
		intval($gid),
		intval($member)
	);
	if(count($r))
 	...
 	if(! count($r))
		$r = q("INSERT INTO `group_member` (`uid`, `gid`, `contact_id`)
			VALUES( %d, %d, %d ) ",
			intval($uid),
			intval($gid),
			intval($member)
...
function group_get_members($gid) {
	$ret = array();
	if(intval($gid)) {
		$r = q("SELECT `group_member`.`contact_id`, `contact`.* FROM `group_member` 
			LEFT JOIN `contact` ON `contact`.`id` = `group_member`.`contact_id` 
...
function group_public_members($gid) {
	$ret = 0;
	if(intval($gid)) {
		$r = q("SELECT `contact`.`id` AS `contact_id` FROM `group_member` 
			LEFT JOIN `contact` ON `contact`.`id` = `group_member`.`contact_id` 
...
function expand_groups($a) {
	...
	$groups = implode(',', $a);
	$groups = dbesc($groups);
	$r = q("SELECT `contact_id` FROM `group_member` WHERE `gid` IN ( $groups )");
	$ret = array();
	if(count($r))
		foreach($r as $rr)
			$ret[] = $rr['contact_id'];
...
function member_of($c) {
	...
	$r = q("SELECT `group`.`name`, `group`.`id` FROM `group` LEFT JOIN `group_member` ON `group_member`.`gid` = `group`.`id` WHERE `group_member`.`contact_id` = %d AND `group`.`deleted` = 0 ORDER BY `group`.`name`  ASC ",
		intval($c)
	);
